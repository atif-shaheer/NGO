...
<!-- Mirrored from sanatandharmajagruti.in/id-card-download.php by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 11 Sep 2024 05:52:10 GMT -->
...
<style>
    ...
    <link rel="stylesheet" href="css/card2602.css?v=1726033886">
    <script type="text/javascript" src="../kendo.cdn.telerik.com/2017.2.621/js/kendo.all.min.js"></script>
    <style>
    @font-face {
        font-family: "DejaVu Sans";
        font-weight: bold;
        src: url("font/Amiko-Regular.ttf") format("truetype");
    }
...
$(document).ready(function() {
    ...
    $('.sidenav2 .dropdown a').each(function() {
        ...
        if (hrefLastPart === lastPart) {
            ...
            <div class="head_item3"><span class="mt-3">Print ID Card</span></div>

    <center>
        <div class="card_container ">
            <div class=" mx-auto main-div mt-3 mb-4">
                <div class="col-md-6 bd">
                    <form id="form" method="post">
                        <div class="form-group">
                            <label for="">User Id :</label>
                            <input type="text" id="user_id" name="Userid" class="form-control" required
                                placeholder="User Id"><br>
                            <label for="">Date Of Birth :</label>
                            <input type="date" name="Dob" class="form-control" value="">
                            <span id='invalid'></span><br>
                            <center>
                                <input type="submit" class="btn btn-warning text-white" id="idcard" name="submit"
                                    required value="ID Card Download">
                                    
                                <input type="submit" class="btn text-white" style="background:var(--background-color)" name="submit1" id="certificate" required value="Appointment Letter">
                                <!--<input type="submit" class="btn btn-secondary" id="certificate" name="submit2" required value="Member Certificate">-->
                            </center>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    $(document).on('click', '#idcard', function(e) {
        ...
        $.ajax({
            url: 'ajax/id-card-download.php',
            type: 'post',
            data: $('#form').serialize(),
            success: function(data) {
                if (data == 2) {
                    var invalid = $('#invalid').html('All fields are required');
                    if ($('#invalid').html() != '') {
                        setTimeout(function() {
                            $('#invalid').html('');
                        }, 3000);
                    ...
                    myRedirect("print-user-card-two.php", "Userid", user_id);
                
                } else if (data == 0) {
                    var invalid = $('#invalid').html(
                        'Invalid Registration Id or Dob / Your Registration Under Verificaton');
                    if ($('#invalid').html() != '') {
                        setTimeout(function() {
                            $('#invalid').html('');
                        }, 3000);
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data == 2) {
                    ...
                    myRedirect("print-certificate-two.php", "Userid", user_id);
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data == 2) {
                    ...
                    myRedirect("print-member-certificate-direct.php", "Userid", user_id);
...
<!-- Mirrored from sanatandharmajagruti.in/id-card-download.php by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 11 Sep 2024 05:52:24 GMT -->
</html>